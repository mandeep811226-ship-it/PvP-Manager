...
// @grant        none
// @veyra-min-tier member
// @veyra-thread   https://discord.com/channels/1426195715746435248/1492529544945471548
// @updateURL    https://script.google.com/macros/s/AKfycbzUHg1z18WmWFSyEsZStaK2kmax2JXnPzK4LrTyEitSFVBQ2u2vfFeO6wZhjWx58EJZ7w/exec/eyJlIjoiY3VyaXZveDEzQGdtYWlsLmNvbSIsInMiOiJwdnAtbWFuYWdlciIsImkiOjE3Nzc4NjkyODB9.2GTUicWL2nvPL7kl3CSP-6LTQbqArxuKW1z2XJZmyCk.user.js
// @downloadURL  https://script.google.com/macros/s/AKfycbzUHg1z18WmWFSyEsZStaK2kmax2JXnPzK4LrTyEitSFVBQ2u2vfFeO6wZhjWx58EJZ7w/exec/eyJlIjoiY3VyaXZveDEzQGdtYWlsLmNvbSIsInMiOiJwdnAtbWFuYWdlciIsImkiOjE3Nzc4NjkyODB9.2GTUicWL2nvPL7kl3CSP-6LTQbqArxuKW1z2XJZmyCk.user.js
// ==/UserScript==
(function(){"use strict";try{var SCRIPT_ID="pvp-manager";var MODE="changelog";var DATA={"name":"PvP Manager","version":"1.17.0","date":"retroactive","notes":"PvP quality-of-life features (background-tab safe, occlusion-safe via silent audio, responsive down to 316px)","author":"[EMP] lmv","threadUrl":"https://discord.com/channels/1426195715746435248/1492529544945471548","archiveUrl":"https://veyra-empire.github.io/scripts/"};
var STORAGE_KEY=MODE==="deletion"?("veyra_deleted_notice_"+SCRIPT_ID):("veyra_seen_"+SCRIPT_ID);
var STORAGE_VAL=MODE==="deletion"?"1":String(DATA.version||"");
var prior=null;try{prior=localStorage.getItem(STORAGE_KEY);}catch(e){return;}
if(prior===STORAGE_VAL)return;
function esc(s){return String(s==null?"":s).replace(/[&<>"']/g,function(c){return({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[c];});}
function renderNotes(raw){
  if(!raw)return '<p style="color:#a39580;margin:0">(no notes provided)</p>';
  var out="";
  String(raw).split(/\n\s*\n/).forEach(function(block){
    var lines=block.split(/\n/).filter(function(l){return l.trim().length;});
    if(lines.length&&lines.every(function(l){return /^\s*[-*]\s+/.test(l);})){
      out+='<ul style="margin:0 0 10px 0;padding-left:22px">';
      lines.forEach(function(l){out+='<li style="margin:2px 0">'+esc(l.replace(/^\s*[-*]\s+/,""))+'</li>';});
      out+='</ul>';
    }else{
      out+='<p style="margin:0 0 10px 0">'+esc(block).replace(/\n/g,'<br>')+'</p>';
    }
  });
  return out;
}
function buildHtml(){
  var head,bodyHtml,footLeft;
  // Small gold section label, sits above the body text so the user can
  // distinguish a version changelog from a first-install welcome from a
  // deletion reason at a glance.
  function sectionLabel(text){
    return '<div style="font-size:12px;font-weight:700;color:#f2c072;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:10px">'+text+'</div>';
  }
  if(MODE==="deletion"){
    head='<div style="font-size:19px;font-weight:600;color:#f2c072;line-height:1.3">'+esc(DATA.name||"")+'</div>'+
         '<div style="font-size:13px;color:#a39580;margin-top:4px">Removed from the archive'+(DATA.retiredAt?" on "+esc(DATA.retiredAt):"")+'</div>';
    var delBody=DATA.reason?('<p style="margin:0">'+esc(DATA.reason).replace(/\n/g,'<br>')+'</p>'):'<p style="margin:0;color:#a39580"><em>No reason provided.</em></p>';
    bodyHtml=sectionLabel("Reason")+delBody;
    var archiveLink=DATA.archiveUrl?('<a href="'+esc(DATA.archiveUrl)+'" target="_blank" rel="noopener noreferrer" style="color:#f2c072;text-decoration:none">Archive \u2192</a>'):"";
    footLeft=(DATA.author?"by "+esc(DATA.author):"")+((DATA.author&&archiveLink)?" \u00b7 ":"")+archiveLink;
  }else{
    head='<div style="font-size:19px;font-weight:600;color:#f2c072;line-height:1.3">'+esc(DATA.name||"")+'</div>'+
         '<div style="font-size:13px;color:#a39580;margin-top:4px">Updated to v'+esc(DATA.version||"?")+(DATA.date?" \u00b7 "+esc(DATA.date):"")+'</div>';
    // prior===null means we've never recorded this script for this browser
    // (first install or retroactive catchup); a non-null prior means the
    // installed version changed and Tampermonkey auto-updated.
    var changelogTitle=prior===null?"Newly installed":"What's new";
    bodyHtml=sectionLabel(changelogTitle)+renderNotes(DATA.notes);
    var discussion=DATA.threadUrl?('<a href="'+esc(DATA.threadUrl)+'" target="_blank" rel="noopener noreferrer" style="color:#f2c072;text-decoration:none">Discussion \u2192</a>'):"";
    footLeft=(DATA.author?"by "+esc(DATA.author):"")+((DATA.author&&discussion)?" \u00b7 ":"")+discussion;
  }
  // Ko-fi donation pill. Croissant emoji nods to lmv's Ko-fi bio ("buy my
  // daughter a croissant"); text stays short ("Support lmv") so the pill
  // fits alongside the Got it button without wrapping on desktop.
  var kofiIcon='<span aria-hidden="true" style="margin-right:6px;font-size:15px;line-height:1">\uD83E\uDD50</span>';
  var kofi='<a href="https://ko-fi.com/lmvdev" target="_blank" rel="noopener noreferrer" '+
           'style="display:inline-flex;align-items:center;background:rgba(242,192,114,0.14);color:#f2c072;'+
           'border:1px solid rgba(242,192,114,0.5);padding:8px 14px;border-radius:999px;'+
           'font-weight:600;font-size:13px;text-decoration:none;white-space:nowrap;font-family:inherit" '+
           'title="Support lmv on Ko-fi">'+kofiIcon+'Support lmv</a>';
  var btn='<button data-veyra-close style="background:#f2c072;color:#0a0612;border:0;padding:9px 18px;border-radius:999px;font-weight:600;cursor:pointer;font-size:14px;font-family:inherit;white-space:nowrap">Got it</button>';
  return '<div style="padding:20px 24px;background:linear-gradient(180deg,#2a1f36,#1a1422);border-bottom:1px solid rgba(242,192,114,0.2)">'+head+'</div>'+
         '<div style="padding:20px 24px;overflow-y:auto;flex:1;min-height:0">'+bodyHtml+'</div>'+
         '<div style="padding:14px 24px;background:#120d1c;border-top:1px solid rgba(242,192,114,0.15);display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:12px">'+
           '<div style="font-size:13px;color:#a39580;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1 1 160px">'+footLeft+'</div>'+
           '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">'+kofi+btn+'</div>'+
         '</div>';
}
function mount(){
  if(!document.body)return;
  if(!window.__veyraCardStack)window.__veyraCardStack=0;
  var idx=window.__veyraCardStack++;var offset=idx*30;
  var root=document.createElement("div");
  root.setAttribute("data-veyra-card",SCRIPT_ID);
  // Width scales down on narrow viewports via min(). Typography is 15px base
  // for readability; the header title uses 19px (see buildHtml). max-height
  // prevents overflow on short viewports while leaving the body scrollable.
  root.style.cssText="position:fixed;z-index:2147483000;top:calc(50% + "+offset+"px);left:calc(50% + "+offset+"px);transform:translate(-50%,-50%) scale(0.96);width:min(560px, calc(100vw - 24px));max-width:560px;max-height:min(85vh, calc(100vh - 32px));background:#1a1422;color:#f5e9d0;border:1px solid rgba(242,192,114,0.3);border-radius:14px;box-shadow:0 28px 72px rgba(0,0,0,0.65);font:15px/1.55 system-ui,-apple-system,'Segoe UI',Roboto,sans-serif;display:flex;flex-direction:column;overflow:hidden;opacity:0;transition:opacity 220ms ease,transform 220ms ease;box-sizing:border-box";
  root.innerHTML=buildHtml();
  document.body.appendChild(root);
  requestAnimationFrame(function(){root.style.opacity="1";root.style.transform="translate(-50%,-50%) scale(1)";});
  root.querySelector("[data-veyra-close]").addEventListener("click",function(){
    root.style.opacity="0";root.style.transform="translate(-50%,-50%) scale(0.96)";
    setTimeout(function(){
      if(root.parentNode)root.parentNode.removeChild(root);
      window.__veyraCardStack=Math.max(0,(window.__veyraCardStack||1)-1);
    },240);
...
(() => {
    ...
    async function runAutomation() {
        await runWithAutoFarmGuard(async () => {
            ...
            try {
                while (!stopFlag) {
                    const shouldContinue = await runOneCycle();
    ...
    async function runOneCycle() {
        ...
        try {
            ...
            addLog(`ERROR: Could not ${strategyOn ? 'fetch initial state' : 'send auto play'} - ${e.message}`, { color: '#ef5350' });
            return false;
        }

        if (!state || state.ok === false) {
            const err = state?.error || 'no details';
            addLog(`ERROR: ${strategyOn ? 'Initial state fetch' : 'Auto play'} rejected - ${err}`, { color: '#ef5350' });
            return false;
        }
